...
function PhotoGallery({ photos, grouped, existingCats, catLabels, catColors, onUpload, getApiBaseUrl }: {
  photos: any[];
  ...
  const [activeCat, setActiveCat] = useState(existingCats[0] || 'ANTES');
  const [lightbox, setLightbox] = useState<string | null>(null);
  const currentPhotos = grouped[activeCat] || [];

  return (
    <>
      {/* Category tabs */}
      <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap', marginBottom: 'var(--space-5)', borderBottom: '1px solid var(--gray-100)', paddingBottom: 'var(--space-3)' }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `1.5px solid ${isActive ? color : 'var(--gray-200)'}`,
            ...
            >
              ...
              <span style={{
                ...
                background: isActive ? color : 'var(--gray-100)',
      ...
      {currentPhotos.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-4)' }}>
          <div style={{ width: 4, height: 20, borderRadius: 2, background: catColors[activeCat] || '#64748b' }} />
          <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--gray-700)' }}>
            {catLabels[activeCat] || activeCat} — {currentPhotos.length} foto{currentPhotos.length !== 1 ? 's' : ''}
          </span>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 'var(--space-3)' }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                animation: `fadeIn 0.3s ease backwards ${i * 40}ms`,
                cursor: 'zoom-in', boxShadow: '0 1px 4px rgba(0,0,0,0.08)',
                transition: 'transform 0.15s ease, box-shadow 0.15s ease',
              }}
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'scale(1.02)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 16px rgba(0,0,0,0.15)'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'scale(1)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 1px 4px rgba(0,0,0,0.08)'; }}
            >
              <img src={src} alt={String(p.description || p.category || 'Foto')} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '6px 8px', background: 'linear-gradient(transparent, rgba(0,0,0,0.65))', color: 'white' }}>
                <div style={{ fontSize: '10px', opacity: 0.85 }}>{new Date(String(p.createdAt)).toLocaleDateString('pt-BR')}</div>
                {p.description && <div style={{ fontSize: '11px', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{String(p.description)}</div>}
              </div>
            </div>
          );
        })}
        ...
        <button
          ...
          style={{
            ...
            border: '2px dashed var(--gray-200)', background: 'var(--gray-25)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
            gap: 'var(--space-2)', cursor: 'pointer', color: 'var(--gray-400)',
            transition: 'all 0.15s ease', fontSize: 'var(--text-xs)',
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = catColors[activeCat] || 'var(--primary-400)'; (e.currentTarget as HTMLButtonElement).style.color = catColors[activeCat] || 'var(--primary-500)'; }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = 'var(--gray-200)'; (e.currentTarget as HTMLButtonElement).style.color = 'var(--gray-400)'; }}
        >
          <Camera size={24} />
          <span>Adicionar</span>
        </button>
      ...
      {lightbox && (
        ...
        >
          <img
            src={lightbox}
            alt="Foto ampliada"
            style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain', borderRadius: 'var(--radius-xl)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}
          />
          <button
            onClick={() => setLightbox(null)}
            style={{
          ...
          {showNewRecord ? (
            ...
            <>
              ...
              {records.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  {[...records].sort((a: any, b: any) => new Date(b.dateTime ?? b.createdAt ?? 0).getTime() - new Date(a.dateTime ?? a.createdAt ?? 0).getTime()).map((r: Record<string, unknown>, i: number) => {
                    const recId = String(r.id);
                    const isExpanded = expandedRecord === recId;
                    const recDate = r.dateTime ?? r.createdAt;
                    const dateObj = recDate ? new Date(String(recDate)) : null;
                    const procLabel = String(r.procedures ?? 'Atendimento');
                    return (
                      <div key={recId} className="card" style={{ animation: `fadeInUp 0.2s ease backwards ${i * 50}ms` }}>
                        <div className="card-body">
                          {/* Summary row — always visible, click to expand */}
                          {!isExpanded ? (
                            <button onClick={() => setExpandedRecord(recId)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
                                {dateObj && (
                                  <div style={{ width: 48, minWidth: 48, height: 48, borderRadius: 'var(--radius-lg)', background: r.isDraft ? 'var(--warning-50, #fffbeb)' : 'var(--primary-50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                                    <div style={{ fontSize: '14px', fontWeight: 'var(--font-bold)', color: r.isDraft ? 'var(--warning-700, #a16207)' : 'var(--primary-700)', lineHeight: 1 }}>{dateObj.getDate().toString().padStart(2, '0')}</div>
                                    <div style={{ fontSize: '9px', color: r.isDraft ? 'var(--warning-500)' : 'var(--primary-500)' }}>{['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][dateObj.getMonth()]}</div>
                                  </div>
                                )}
                                <div>
                                  <div style={{ fontWeight: 'var(--font-medium)', fontSize: 'var(--text-sm)', color: 'var(--gray-900)' }}>{procLabel}</div>
                                  <div style={{ fontSize: 'var(--text-xs)', color: 'var(--gray-400)' }}>
                                    {dateObj?.toLocaleDateString('pt-BR', { weekday: 'long' })}
                                    {r.complaint ? ` · ${String(r.complaint).substring(0, 40)}${String(r.complaint).length > 40 ? '...' : ''}` : ''}
                                  </div>
                                </div>
                              </div>
                            ...
                            <div style={{ animation: 'fadeIn 0.2s ease' }}>
                              {/* Header */}
                              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-5)' }}>
                                <button className="btn btn-ghost btn-sm btn-icon" onClick={() => setExpandedRecord(null)}><ChevronLeft size={18} /></button>
                                <h3 style={{ fontSize: 'var(--text-base)', fontWeight: 600 }}>Detalhes do Atendimento</h3>
                                <span className={`badge badge-dot ${r.isDraft ? 'badge-warning' : 'badge-success'}`} style={{ marginLeft: 'auto' }}>{r.isDraft ? 'Rascunho' : 'Finalizado'}</span>
                              </div>
                              {/* Date + procedure row */}
                              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-5)', padding: 'var(--space-3)', background: 'var(--primary-50)', borderRadius: 'var(--radius-lg)' }}>
                                <div style={{ width: 44, height: 44, borderRadius: 'var(--radius-md)', background: 'var(--primary-100)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flexShrink: 0 }}>
                                  <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--primary-700)', lineHeight: 1 }}>{dateObj?.getDate().toString().padStart(2, '0')}</div>
                                  <div style={{ fontSize: '9px', color: 'var(--primary-500)' }}>{dateObj ? ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][dateObj.getMonth()] : ''}</div>
                                </div>
                                <div>
                                  <div style={{ fontWeight: 600, color: 'var(--primary-800)', fontSize: 'var(--text-sm)' }}>{procLabel}</div>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                                {Boolean(r.complaint) && (
                                  <div style={{ padding: 'var(--space-3)', background: 'var(--gray-25)', borderRadius: 'var(--radius-lg)' }}>
                                    <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--gray-400)', textTransform: 'uppercase', marginBottom: 4 }}>Queixa Principal</div>
                                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--gray-700)', margin: 0 }}>{String(r.complaint)}</p>
                                  </div>
                                )}
                                {Boolean(r.diagnosis) && (
                                  <div style={{ padding: 'var(--space-3)', background: 'var(--gray-25)', borderRadius: 'var(--radius-lg)' }}>
                                    <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--gray-400)', textTransform: 'uppercase', marginBottom: 4 }}>Diagnóstico</div>
                                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--gray-700)', margin: 0 }}>{String(r.diagnosis)}</p>
                                  </div>
                                )}
                                {Boolean(r.treatment) && (
                                  <div style={{ padding: 'var(--space-3)', background: 'var(--gray-25)', borderRadius: 'var(--radius-lg)' }}>
                                    <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--gray-400)', textTransform: 'uppercase', marginBottom: 4 }}>Tratamento / Evolução</div>
                                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--gray-700)', margin: 0 }}>{String(r.treatment)}</p>
                                  </div>
                                )}
                                {Boolean(r.prescription) && (
                                  <div style={{ padding: 'var(--space-3)', background: 'var(--gray-25)', borderRadius: 'var(--radius-lg)' }}>
                                    <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--gray-400)', textTransform: 'uppercase', marginBottom: 4 }}>Prescrição / Receita</div>
                                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--gray-700)', margin: 0, whiteSpace: 'pre-wrap' }}>{String(r.prescription)}</p>
                                  </div>
                                )}
                                {Boolean(r.notes) && (
                                  <div style={{ padding: 'var(--space-3)', background: 'var(--gray-25)', borderRadius: 'var(--radius-lg)' }}>
                                    <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--gray-400)', textTransform: 'uppercase', marginBottom: 4 }}>Observações Internas</div>
                                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--gray-700)', margin: 0 }}>{String(r.notes)}</p>
                                  </div>
                                )}
                                {Boolean(r.nextReturn) && (
                                  <div style={{ padding: 'var(--space-3)', background: 'var(--success-50, #f0fdf4)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--success-200, #bbf7d0)' }}>
                                    ...
                                    <p style={{ fontSize: 'var(--text-sm)', color: 'var(--success-700)', margin: 0, fontWeight: 600 }}>{new Date(String(r.nextReturn)).toLocaleDateString('pt-BR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                  </div>
                                )}
                              </div>
                            </div>
          ...
          {showUploadPhoto ? (
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
                ...
                <button className="btn btn-primary btn-sm" onClick={() => setShowUploadPhoto(true)}><Camera size={14} /> Adicionar Fotos</button>
              </div>
              ...
              {photos.length === 0 ? (
                ...
                const CAT_LABELS: Record<string, string> = {
                  ...
                  RAIO_X: 'Raio-X', TOMOGRAFIA: 'Tomografia', OUTRO: 'Outro',
                };
                const CAT_COLORS: Record<string, string> = {
                  ANTES: '#2563eb', DURANTE: '#d97706', DEPOIS: '#16a34a',
                  RAIO_X: '#7c3aed', TOMOGRAFIA: '#0891b2', OUTRO: '#64748b',
                };
                const CAT_ORDER = ['ANTES', 'DURANTE', 'DEPOIS', 'RAIO_X', 'TOMOGRAFIA', 'OUTRO'];
                const grouped = photos.reduce((acc: Record<string, any[]>, p: any) => {
                  const cat = String(p.category || 'OUTRO');
                  if (!acc[cat]) acc[cat] = [];
                  acc[cat].push(p);
                  return acc;
                }, {});
                const existingCats = CAT_ORDER.filter(c => grouped[c]?.length > 0);
                // Use first category as active if no state
                const [activeCat, setActiveCat] = [photos[0] ? (grouped['ANTES']?.length > 0 ? 'ANTES' : existingCats[0]) : 'ANTES', () => {}];
                return (
                  <PhotoGallery photos={photos} grouped={grouped} existingCats={existingCats} catLabels={CAT_LABELS} catColors={CAT_COLORS} onUpload={() => setShowUploadPhoto(true)} getApiBaseUrl={getApiBaseUrl} />
                );
              })()}
            </>
          )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexShrink: 0 }}>
                            ...
                            {anamnese.status === 'RASCUNHO' ? (
                              ...
                              <button className="btn btn-ghost btn-sm btn-icon" title="Ver / Editar" onClick={() => setEditingAnamnese(anamnese)}>
