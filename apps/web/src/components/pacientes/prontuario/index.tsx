        ...
        {!isExpanded ? (
          <button onClick={onToggle} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
              {dateObj && <DateBlock date={dateObj} variant={record.isDraft ? 'warning' : 'primary'} />}
              <div>
                <div style={{ fontWeight: 'var(--font-medium)', fontSize: 'var(--text-sm)' }}>{procLabel}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
              <span className={`badge badge-dot ${record.isDraft ? 'badge-warning' : 'badge-success'}`}>{record.isDraft ? 'Rascunho' : 'Finalizado'}</span>
          ...
          <div style={{ animation: 'fadeIn 0.2s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-5)' }}>
              ...
              <span className={`badge badge-dot ${record.isDraft ? 'badge-warning' : 'badge-success'}`} style={{ marginLeft: 'auto' }}>{record.isDraft ? 'Rascunho' : 'Finalizado'}</span>
            </div>
