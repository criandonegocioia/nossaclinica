...
function Gallery({ photos, onUpload }: { photos: Photo[]; onUpload: () => void }) {
  ...
  const existing = (Object.keys(PHOTO_CATEGORIES) as PhotoCategory[]).filter((c) => grouped[c]?.length > 0);
  const [activeCat, setActiveCat] = useState<PhotoCategory>(existing[0] || 'ANTES');
  const [lightbox, setLightbox] = useState<string | null>(null);
  const current = grouped[activeCat] || [];
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap', marginBottom: 'var(--space-5)', borderBottom: '1px solid var(--gray-100)', paddingBottom: 'var(--space-3)' }}>
        {existing.map((cat) => {
          ...
          return (
            <button key={cat} onClick={() => setActiveCat(cat)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 'var(--radius-full)', border: `1.5px solid ${active ? color : 'var(--gray-200)'}`, background: active ? `${color}12` : 'white', color: active ? color : 'var(--gray-500)', fontSize: 'var(--text-sm)', fontWeight: active ? 600 : 400, cursor: 'pointer', transition: 'all 0.15s ease' }}>
              {label}
              <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 18, height: 18, borderRadius: '50%', background: active ? color : 'var(--gray-100)', color: active ? 'white' : 'var(--gray-500)', fontSize: 10, fontWeight: 600 }}>{grouped[cat]?.length ?? 0}</span>
            </button>
          );
        })}
      </div>
...
export default function FotosTab({ patientId }: TabComponentProps) {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.2s ease' }}>
      ...
      {photos.length === 0 ? (
        <EmptyState icon={Camera} message="Nenhuma foto cadastrada"
          action={<button className="btn btn-primary btn-sm" onClick={() => setShowUpload(true)}><Upload size={14} /> Adicionar Foto</button>} />
      ) : (
